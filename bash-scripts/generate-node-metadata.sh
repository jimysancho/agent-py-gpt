...
info_file="../$2"
cd bash-scripts
./classify-python-code.sh $python_file $info_file
file=$info_file 
  ...
  if [[ $keyword == "Function:" ]]; then 
    ...
    if [[ -n $function_names ]]; then 
      while read function_name; do 
        ...
        sed -n "${first_line_of_code},${last_line_of_code}p" $code_file
    ...
    if [[ -n $class_names ]]; then 
      while read class_name; do 
        ...
        while read method_line ; do 
          ...
          sed -n "${method_definition_line},${method_end_line}p" $code_file
        ...
        sed -n "${class_definition_line},${method_end_line}p" $code_file
     ...
     if [[ -n $class_names ]]; then 
      while read class_name; do 
        ...
        sed -n "${begin_class_line},${end_class_line}p" $code_file
        echo "#- END CLASS: $class_name #-"
      done <<< "$class_names"
     fi  
    fi 
    ...
    if [[ -n $end_module_line ]]; then 
      echo "#- BEGIN MODULES -#"
      sed -n "${begin_module_line},${end_module_line}p" $code_file
      echo "#- END MODULES -#"
    fi 
  fi
done 
...
while read file_line; do 
  ...
  if [[ -n $is_code_line && -z $begin_block ]]; then 
    ...
    sed -n "${begin_block},${end_code_block}p" $code_file
